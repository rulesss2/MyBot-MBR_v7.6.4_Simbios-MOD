...
Func SplashStep($status, $bIncreaseStep = True)
	...
	Local Static $tookTotalTime
	If ($tookTotalTime == "") Then $tookTotalTime = 0
	Local $tookTime = Round(__TimerDiff($g_hSplashTimer) / 1000, 2)
	$tookTotalTime += $tookTime
	SetDebugLog("SplashStep " & $g_iSplashCurrentStep & " of " & $g_iSplashTotalSteps & ": " & $status & "(" & $tookTime & " sec)")
	If $g_iSplashCurrentStep == $g_iSplashTotalSteps Then SetDebugLog("It Took (" & $tookTotalTime & " sec) To Run SM MOD")
	;------------------ADDED By SM MOD - END------------------
