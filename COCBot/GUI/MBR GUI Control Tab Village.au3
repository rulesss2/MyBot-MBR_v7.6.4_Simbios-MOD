...
Func chkRequestCountCC()
	...
	Else
		...
		;GUICtrlSetState($g_hTxtClanCastleSpell, $GUI_DISABLE) ;Removed By SM MOD
	EndIf
...
Func cmbClanCastleSpell()
	...
	If $iSpellSelection < 5 Then
		...
		;GUICtrlSetState($g_hTxtClanCastleSpell, BitOR($GUI_HIDE, $GUI_DISABLE));Removed By SM MOD
	Else
		GUICtrlSetState($g_hCmbClanCastleSpell2, BitOR($GUI_SHOW, $GUI_ENABLE))
		;GUICtrlSetState($g_hTxtClanCastleSpell, BitOR($GUI_SHOW, $GUI_ENABLE));Removed By SM MOD
	EndIf
...
;------------------ADDED By SM MOD - START------------------
; Request CC Troops at first - SM MOD
Func chkReqCCFirst()
