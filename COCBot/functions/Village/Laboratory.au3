...
Func Laboratory()
	...
	If $iCheck = 0 And $g_bChkPrioritySystem = True Then
...
Func LabPriority()
	...
	If $g_bChkPrioritySystem = True Then
		...
		For $iz = 0 to 31
			...
			EndIf
		Next
		Setlog("Max Troop/Spell/Seige Count: " & $iMaxCount, $COLOR_INFO)
			...
			If ($iLabMax[$iz][$g_iLabLevel] < IniRead($g_sProfileConfigPath, $g_iUpgradeLevel[$iz][0], $g_iUpgradeLevel[$iz][1], 0)) Then
				Switch $iz
					Case 1 To 13
						ContinueCase
					Case 24 To 30
						$g_aiTrainArmyTroopLevel[($g_iUpgradeLevel[$g_iCmbLaboratory - 1][3])] = 1
						LevUpDownTroop($g_iUpgradeLevel[$g_iCmbLaboratory - 1][3], False)
					Case 14 To 23
						$g_aiTrainArmySpellLevel[($g_iUpgradeLevel[$g_iCmbLaboratory - 1][3])] = 1
						LevUpDownSpell($g_iUpgradeLevel[$g_iCmbLaboratory - 1][3], False)
					Case 31 To 32
						$g_aiTrainArmySiegeMachineLevel[($g_iUpgradeLevel[$g_iCmbLaboratory - 1][3])] = 1
						LevUpDownSiege($g_iUpgradeLevel[$g_iCmbLaboratory - 1][3], False)
				EndSwitch
			EndIf
		Next
