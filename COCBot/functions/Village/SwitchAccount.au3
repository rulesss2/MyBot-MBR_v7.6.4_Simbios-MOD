	...
	Else
		...
		If $g_iNextAccount <> $g_iCurAccount Then
			If $g_bRequestTroopsEnable And $g_bCanRequestCC Then
				If _Sleep(1000) Then Return ;OFFICAL BUG FIX ADDED BY SM MOD
		...
		Else
			...
			SetSwitchAccLog("Stay at [" & $g_iCurAccount + 1 & "]", $COLOR_SUCCESS)
			VillageReport() ; Added By SM Mod
			CheckFarmSchedule() ; Added By SM Mod
		EndIf
	EndIf
...
Func CheckGoogleSelectAccount($bSelectFirst = True)
	...
	If _ColorCheck($pColor, Hex($aListAccount[2], 6), $aListAccount[3]) Then ; White
		...
		If UBound(decodeSingleCoord(FindImageInPlace("GoogleSelectAccount", $g_sImgGoogleSelectAccount, "180,40(395,360)", False))) > 1 Then
