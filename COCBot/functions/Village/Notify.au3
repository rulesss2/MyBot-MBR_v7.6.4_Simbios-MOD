...
Func NotifyActivateKeyboardOnTelegram($TGMsg)
	...
	If IsObj($oHTTP) Then
		...
		If (@error) Then
			SetLog("Telegram __HttpGet/Post Error", $COLOR_ERROR)
			Return False
		EndIf
		...
		Local $TGPushMsg = '{"text": "' & $TGMsg & '", "chat_id":' & $g_sTGChatID & ', "reply_markup":' & $ReplayMarkup & '}'

		$oHTTP.Send($TGPushMsg)
		If (@error) Then
			SetLog("Telegram Keyboard/Send Error", $COLOR_ERROR)
			Return False
		EndIf
