...
Func AttackBarCheck($bRemaining = False, $pMatchMode = $DB, $bDebug = False)
	...
	Static Local $CheckSlotwAfterHero = False

	If Not $bRemaining Then
		$bCheckSlot12 = False
		$bCheckSlotwHero = False
		$CheckSlotwAfterHero = False
	EndIf
	...
	If IsArray($sAttBarRes) Then
		...
		Else
			$sAttBarRes = logicForAllowingDropEmptySeige($sAttBarRes) ; Added By SM MOD
			...
			If Not $bRemaining Then
				...
				For $i = 0 To UBound($aResult) - 1
					If $aResult[$i][0] = "King" Or $aResult[$i][0] = "Queen" Or $aResult[$i][0] = "Warden" Then $bCheckSlotwHero = True
				Next
			EndIf
			...
			For $i = 0 To UBound($aResult) - 1
				...
				If $aResult[$i][1] > 0 Then
					...
					; JUST ONLY AFTER HEROES NEED MORE -- 14px or 15px --
					If $aResult[$i][0] = "King" Or $aResult[$i][0] = "Queen" Or $aResult[$i][0] = "Warden" Then $CheckSlotwAfterHero = True
					; IF $bCheckSlot12 the slot0 will start 16 w/o Heroes
					$aTempSlot = SlotAttack(Number($aResult[$i][1]), $bCheckSlot12, $bCheckSlotwHero, $CheckSlotwAfterHero)
					...
					If UBound($aTempSlot) = 2 Then
						...
						Else
							...
							If StringInStr($aResult[$i][0], "ESpell") <> 0 And $g_bSmartZapEnable Then
								$aResult[$i][5] = getTroopsSpellsLevel(Number($aTempSlot[0]) + $iSPellLevelOCR, 704)
...
Func SlotAttack($PosX, $CheckSlot12, $CheckSlotwHero, $CheckSlotwAfterHero)

	Local $Slottemp[2] = [0, 0]
	; Regular slot0 is 37px
	Local $slot0 = 37

	; If exist heroes and less than 10 slots the slot0 is : 29px
	If $CheckSlotwHero And $CheckSlot12 = False Then $slot0 = 29
	; If is more than 11 slots the slot0 will be always 16px
	If $CheckSlot12 Then $slot0 = 16

	For $i = 0 To 12
		If $PosX >= $slot0 + ($i * 73) And $PosX < 102 + ($i * 73) Then
			; 65 is the slot size , 53 is the Ocr size and $i is a compensation , slots & spaces are different when scrolling through the bar
			$Slottemp[0] = $slot0 + ($i * 73) + (65 - 53 - $i)
			$Slottemp[1] = $i
			;[7+8] sometimes is 14 and others is 15 , we have that compensation $i above
			If $CheckSlotwAfterHero Then $Slottemp[0] += 15
			If $g_bDebugSetlog Then SetDebugLog("Slot: " & $i & " | $x > " & $slot0 + ($i * 73) & " and $x < " & 98 + ($i * 73))
