...
Func DropOnPixel($troop, $listArrPixel, $number, $slotsPerEdge = 0, $Random = False) ; EDITED By SM MOD
	...
	For $i = 0 To UBound($listArrPixel) - 1
		...
		While ($nbTroopsLeft > 0)
			...
			For $j = 0 To UBound($arrPixel) - 1 Step $offset
				...
				If $Random Then  $currentPixel = DeployPointRandom($arrPixel[Floor($index)]) ; ADDED By SM MOD
...
Func DeployPointRandom($currentPixel)
	...
	Local $Side = Side($currentPixel)
	Local $x = Random(10, 20, 1)
	Local $y = Random(10, 20, 1)

	Switch $Side
		Case "TL"
			$currentPixel[0] = $currentPixel[0] - $x
			$currentPixel[1] = $currentPixel[1] - $y
		Case "TR"
			$currentPixel[0] = $currentPixel[0] + $x
			$currentPixel[1] = $currentPixel[1] - $y
		Case "BL"
