...
Func getArmyHeroTime($iHeroType, $bOpenArmyWindow = False, $bCloseArmyWindow = False)
	...
	For $index = 0 To UBound($aHeroRemainData) - 1 ;cycle through all 3 slots and hero types
		...
		If $sResult <> "" Then
			...
			$aResultHeroes[$index] = HeroBoostTimeDiv($aResultHeroes[$index], $index) ;Added By SM MOD FOR Getting Heroes Boost Time In Smart Way
	...
	ElseIf StringInStr($iHeroType, "all", $STR_NOCASESENSEBASIC) > 0 Then
		...
		For $i = 0 To 2
			If $aResultHeroes[$i] <> "" And $aResultHeroes[$i] > 0 Then $g_asHeroHealTime[$i] = _DateAdd("s", Int($aResultHeroes[$i] * 60), _NowCalc()) ; EDITED BY SM MOD Bug fix if hero heal time is in float in case of boost or time difference this was returning zero converted into Int
