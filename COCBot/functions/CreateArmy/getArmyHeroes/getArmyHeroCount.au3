...
;------------------ADDED By SM MOD - START------------------
	If $g_iLabLevel < 10 Then
		GetLabLevel(True) ; Good place to check Lab Level while checking status
	EndIf
;------------------ADDED By SM MOD - END------------------	
	...
	If $g_aiLaboratoryPos[0] <= 0 Or $g_aiLaboratoryPos[1] <= 0 Then
		...
		EndIf
	EndIf
	If _Sleep(300) Then Return ; EDITED BY SM MOD Sometimes do not open lab for check
...
;------------------CUSTOM LOGIC By SM MOD Official Bug Fix - START------------------
Func HideShields($bHide = False)
	Local Static $ShieldState[19]
	Local $counter
	If $bHide = True Then
		$counter = 0
		For $i = $g_hlblKing To $g_hPicLabRed ; Official Bug was $g_hPicLabGreen Should be Because $g_hPicLabRed is the last GUI Item
	...
	Else
		$counter = 0
		Local $isLabShieldShown = False
		Local $isKingShieldShown = False
		Local $isQueenShieldShown = False
		Local $isWardenShieldShown = False
		For $i = $g_hlblKing To $g_hPicLabRed ; Official Bug was $g_hPicLabGreen Should be Because $g_hPicLabRed is the last GUI Item
			If $ShieldState[$counter] = 80 Then
				GUICtrlSetState($i, $GUI_SHOW)
			ElseIf $i = $g_hlblKing Or $i = $g_hlblQueen Or $i = $g_hlblWarden Or $i = $g_hlblLab Then ;If any of this is hidden show them
				GUICtrlSetState($i, $GUI_SHOW)
			EndIf
			; Logic to check if Bot is able to show there shields or not because (autoit does not provide any specific method to show or hide GUI items) this above logic is just a hack which can fail
			If ($i = $g_hPicKingGray Or $i = $g_hPicKingBlue Or $i = $g_hPicKingGreen Or $i = $g_hPicKingRed) And $ShieldState[$counter] = 80 Then $isKingShieldShown = True
			If ($i = $g_hPicQueenGray Or $i = $g_hPicQueenBlue Or $i = $g_hPicQueenGreen Or $i = $g_hPicQueenRed) And $ShieldState[$counter] = 80 Then $isQueenShieldShown = True
			If ($i = $g_hPicWardenGray Or $i = $g_hPicWardenBlue Or $i = $g_hPicWardenGreen Or $i = $g_hPicWardenRed) And $ShieldState[$counter] = 80 Then $isWardenShieldShown = True
			If ($i = $g_hPicLabGray Or $i = $g_hPicLabGreen Or $i = $g_hPicLabRed) And $ShieldState[$counter] = 80 Then $isLabShieldShown = True
			$counter += 1
		Next

		If Not $isKingShieldShown Then
			If $g_iHeroUpgrading[0] = 1 Then GUICtrlSetState($g_hPicKingRed, $GUI_SHOW) ;Set king shield to upgrade
			If $g_iHeroUpgrading[0] = 0 Then GUICtrlSetState($g_hPicKingGray, $GUI_SHOW) ;Reset shield status to gray
		EndIf
		If Not $isQueenShieldShown Then
			If $g_iHeroUpgrading[1] = 1 Then GUICtrlSetState($g_hPicQueenRed, $GUI_SHOW) ;Set queen shield to upgrade
			If $g_iHeroUpgrading[1] = 0 Then GUICtrlSetState($g_hPicQueenGray, $GUI_SHOW) ;Reset shield status to gray
		EndIf
		If Not $isWardenShieldShown Then
			If $g_iHeroUpgrading[2] = 1 Then GUICtrlSetState($g_hPicWardenRed, $GUI_SHOW) ;Set warden shield to upgrade
			If $g_iHeroUpgrading[2] = 0 Then GUICtrlSetState($g_hPicWardenGray, $GUI_SHOW) ;Reset shield status to gray
		EndIf
		If Not $isLabShieldShown Then
			If $g_iLabUpgradeProgress = 1 Then GUICtrlSetState($g_hPicLabGreen, $GUI_SHOW) ;Set Lab shield status to green
			If $g_iLabUpgradeProgress = 0 Then GUICtrlSetState($g_hPicLabGray, $GUI_SHOW) ;Reset shield status to gray
		EndIf
	EndIf
EndFunc   ;==>HideShields
;------------------CUSTOM LOGIC By SM MOD Official Bug Fix - END------------------