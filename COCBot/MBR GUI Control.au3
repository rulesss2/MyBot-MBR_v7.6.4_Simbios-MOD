...
Func InitializeMainGUI($bGuiModeUpdate = False)
	...
	If $g_bDevMode Then
		...
		;GUICtrlSetState($g_hbtnAttNow, $GUI_SHOW + $GUI_ENABLE) REMOVED BY SM MOD
	EndIf
...
Func UpdateFrmBotStyle()
	...
	#cs Works but causes bot window not to get activated anymore
		...
		Return
	#ce Works but causes bot window not to get activated anymore
	;Local $ShowMinimize = $g_bAndroidBackgroundLaunched = True Or $g_bAndroidEmbedded = False Or ($g_bAndroidEmbedded = True And $g_bAndroidAdbScreencap = True And $g_bChkBackgroundMode = True)
...
Func GUIControl_WM_NOTIFY($hWind, $iMsg, $wParam, $lParam)
	...
	Switch $nID
		Case $g_hTabMain
			...
			tabMain()
			showForecast($lParam, False) ;If Forecast Was Already Drawn Shown if timeout happend then resend request for forecast - ADDED By SM MOD
		...
		Case $g_hGUI_THSNIPE_TAB
			tabTHSnipe()
		Case $g_hGUI_MOD_TAB ; ADDED By SM Mod
			tabMOD($lParam) ; $lParam is for Forecast Mod - ADDED By SM MOD
		Case $g_hGUI_BOT_TAB
			tabBot()
		Case $g_hGUI_SWITCH_OPTIONS_TAB ; ADDED By SM Mod
			tabSwitchOptions() ; ADDED By SM Mod
		Case Else
...
Func BotGuiModeToggle()
	...
	Switch $g_iGuiMode
		...
		Case 1
			...
			GUICtrlDelete($g_hTabMOD) ; ADDED By SM MOD
			...
			GUICtrlDelete($g_hGUI_MOD_TAB) ; ADDED By SM MOD
			GUICtrlDelete($g_hGUI_BOT_TAB)
			GUICtrlDelete($g_hGUI_LOG_SA)
			GUICtrlDelete($g_hGUI_SWITCH_OPTIONS_TAB) ; ADDED By SM MOD
...
Func tabMain()
	...
	Select
		Case $tabidx = 0 ; Log
			...
			GUISetState(@SW_HIDE, $g_hGUI_MOD) ; ADDED By SM MOD
		...
		Case $tabidx = 3 ; MOD
			...
			tabMOD()
			;------------------ADDED By SM MOD - END------------------
		Case $tabidx = 4 ; Options ; EDITED By SM MOD
			GUISetState(@SW_HIDE, $g_hGUI_LOG)
			GUISetState(@SW_HIDE, $g_hGUI_VILLAGE)
			GUISetState(@SW_HIDE, $g_hGUI_ATTACK)
			GUISetState(@SW_HIDE, $g_hGUI_MOD) ; ADDED By SM MOD
		...
		Case $tabidx = 5 ; About ; EDITED By SM MOD
			GUISetState(@SW_HIDE, $g_hGUI_LOG)
			GUISetState(@SW_HIDE, $g_hGUI_VILLAGE)
			GUISetState(@SW_HIDE, $g_hGUI_ATTACK)
			GUISetState(@SW_HIDE, $g_hGUI_MOD) ; ADDED By SM MOD
...
Func tabMOD($lParam = "")
	...
	Select
		...
		Case $tabidx = 1 ; GTFO Tab

		Case $tabidx = 2 ; War Preparation Tab

		Case $tabidx = 3 ; ChatBot Tab

		Case $tabidx = 4 ; Goblin XP Tab

		Case $tabidx = 5 ; Forecast Tab
			If $lParam = "" Then
				redrawForecast() ;Show already saved offline Forecast -ADDED By SM MOD
				Return
			EndIf
			showForecast($lParam, True) ; Show Loader and ask for forecast update
			showForecast($lParam, False) ; Draw forecast feteched from internet
		Case $tabidx = 6 ; Switch Profiles Tab

		Case $tabidx = 7 ; SM Debug Tab

	EndSelect
EndFunc   ;==>tabMOD

Func showForecast($lParam, $showLoader = False)

	If GUICtrlRead($g_hGUI_MOD_TAB, 1) = $g_hGUI_MOD_TAB_FORECAST Then
		...
		If $iCode = -551 Then ;tab selected
			GUICtrlSetState($g_hGUI_MOD_TAB_FORECAST, $GUI_SHOW)
			If TimerDiff($TimerForecast) > (5 * 60 * 1000) Then ; 1 Refresh Graph every 5 minutes max, do not override
				Sleep(100)
				If $showLoader Then setForecast() ; Show Loader
				If Not $showLoader Then
					cmbSwLang() ; Hide Loader And Show Forecast
					$TimerForecast = TimerInit()
				EndIf
			EndIf
		EndIf
	EndIf
EndFunc   ;==>showForecast
;------------------ADDED By SM MOD - END------------------
...
Func tabBot()
	...
	Select
		Case $tabidx = 0 ; Options tab
			...
			GUISetState(@SW_HIDE, $g_hGUI_SWITCH_OPTIONS) ; EDITED By SM Mod was $g_hGUI_LOG_SA
		...
		Case $tabidx = 3 ; Profiles tab
			GUISetState(@SW_HIDE, $g_hGUI_STATS)
			GUISetState(@SW_SHOWNOACTIVATE, $g_hGUI_SWITCH_OPTIONS) ; EDITED By SM Mod was $g_hGUI_LOG_SA
			ControlHide("", "", $g_hCmbGUILanguage)
			tabSwitchOptions() ; ADDED By SM Mod
...
Func Bind_ImageList($nCtrl, ByRef $hImageList)
	...
	Switch $nCtrl
		Case $g_hTabMain
			...
			Local $aIconIndex = [$eIcnHourGlass, $eIcnTH12, $eIcnAttack, $eIcnSMMod, $eIcnGUI, $eIcnInfo] ; EDITED By SM MOD
		...
		Case $g_hGUI_MISC_TAB
			Local $aIconIndex = [$eIcnTH1, $eIcnBuilderHall, $eIcnClanGames] ; EDITED By SM MOD
		...
		Case $g_hGUI_ATTACKOPTION_TAB
			...
			;----------- ADDED BY SM MOD Starts----------------
		Case $g_hGUI_MOD_TAB
			; the icons for Mods tab
			Local $aIconIndex = [$eIcnModHumanization, $eIcnModGTFO, $eIcnModWar, $eIcnModChatBot, $eIcnModGoblinXp, $eIcnModForecast, $eIcnModSwitchProfile, $eIcnModDebug]
			;----------- ADDED BY SM MOD END ----------------
		Case $g_hGUI_BOT_TAB
			; the icons for Bot tab
			Local $aIconIndex = [$eIcnOptions, $eIcnAndroid, $eIcnModDebug, $eIcnProfile, $eIcnGold] ; EDITED By SM MOD
		...
		Case $g_hGUI_SWITCH_OPTIONS_TAB
			...
			Local $aIconIndex = [$eIcnModSwitchProfile, $eIcnFarmingSchedule]
			;----------- ADDED BY SM MOD END ----------------
