...
Func CheckTrainingTab($sText = "Troops")
	...
	If $sText = "Troops" Then
		If Not OpenTroopsTab(False, "CheckTrainingTab") Then $g_sSmartTrainError = SetError(2, 0, "Error OpenTroopsTab called from CheckTrainingTab")
	Else
		If Not OpenSpellsTab(False, "CheckTrainingTab") Then $g_sSmartTrainError = SetError(2, 0, "Error OpenSpellsTab called from CheckTrainingTab")
	EndIf
...
Func ClearTrainingArmyCamp($OpenTabNumber = "Current", $bReturnArmyTab = False)
	...
	If $OpenTabNumber = "Troop" Then
		If Not OpenTroopsTab(False, "ClearTrainingArmyCamp") Then $g_sSmartTrainError = SetError(3, 0, "Error OpenTroopsTab called from ClearTrainingArmyCamp")
	ElseIf $OpenTabNumber = "Spell" Then
		If Not OpenSpellsTab(False, "ClearTrainingArmyCamp") Then $g_sSmartTrainError = SetError(3, 0, "Error OpenSpellsTab called from ClearTrainingArmyCamp")
	EndIf
	If @error Then Return ; quit SmartTrain

	If _ColorCheck(_GetPixelColor(825, 204, True), Hex(0xCFCFC8, 6), 15) Then Return False ; Gray background found, no troop is training

	If DragNDropQueue() Then Return ; Call Drag and drop when sure that there are some troops in queue

	Local $x = 0
	While Not _ColorCheck(_GetPixelColor(825, 204, True), Hex(0xCFCFC8, 6), 15) ; the gray background at slot 0 troop
