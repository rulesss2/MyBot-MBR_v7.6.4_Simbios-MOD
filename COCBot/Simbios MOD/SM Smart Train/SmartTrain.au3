...
Func MakeCustomTrain($sText, $aeMethod)
	...
	If $sText <> "Spells" Then
		If Not OpenTroopsTab(True,"MakeCustomTrain") Then $g_sSmartTrainError = SetError(2, 0, "Error OpenTroopsTab called from MakeCustomTrain")
	...
	If $sText <> "Troops" Then
		If Not OpenSpellsTab(True,"MakeCustomTrain") Then $g_sSmartTrainError = SetError(2, 0, "Error OpenSpellsTab called from MakeCustomTrain")
		If @error Then Return ; quit SmartTrain
...
Func SmartTrainSiege()
	If $g_bDebugSetlog Then SetLog("Inside SmartTrainSiege $g_iTotalTrainSpaceSiege:"&$g_iTotalTrainSpaceSiege, $COLOR_DEBUG)
