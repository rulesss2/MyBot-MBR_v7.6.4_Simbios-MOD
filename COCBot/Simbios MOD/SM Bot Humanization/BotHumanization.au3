...
Func BotHumanization()
	If $g_bUseBotHumanization = True Then
		...
		SetLog("Now, let the SM MOD Team make your BOT more human ... :)", $COLOR_SUCCESS1)
		...
		If $NoActionsToDo <> 13 Then
			...
			SetLog("SM MOD will do " & $g_iMaxActionsNumber & " human actions during this loop...", $COLOR_INFO)
...
Func RandomHumanAction()
	...
	Switch $g_iActionToDo
		Case 0
			SetLog("The spirit of SM MOD choose to read Clan Chat... Let's go !!! :)", $COLOR_INFO)
			ReadClanChat()
		Case 1
			SetLog("The spirit of SM MOD choose to read Global Chat... Let's go !!! :)", $COLOR_INFO)
			ReadGlobalChat()
		Case 2
			SetLog("The spirit of SM MOD choose to talk with your Clan... Let's go !!! :)", $COLOR_INFO)
			SaySomeChat()
		Case 3
			SetLog("The spirit of SM MOD choose to Watch a Defense... Let's go !!! :)", $COLOR_INFO)
			WatchDefense()
		Case 4
			SetLog("The spirit of SM MOD choose to Watch an Attack... Let's go !!! :)", $COLOR_INFO)
			WatchAttack()
		Case 5
			SetLog("The spirit of SM MOD choose to Look at War Log... Let's go !!! :)", $COLOR_INFO)
			LookAtWarLog()
		Case 6
			SetLog("The spirit of SM MOD choose to Visit Clanmates... Let's go !!! :)", $COLOR_INFO)
			VisitClanmates()
		Case 7
			SetLog("The spirit of SM MOD choose to Visit Best Players... Let's go !!! :)", $COLOR_INFO)
			VisitBestPlayers()
		Case 8
			SetLog("The spirit of SM MOD choose to Look at Best Clans... Let's go !!! :)", $COLOR_INFO)
			LookAtBestClans()
		Case 9
			SetLog("The spirit of SM MOD choose to Look at Current War... Let's go !!! :)", $COLOR_INFO)
			LookAtCurrentWar()
		Case 10
			SetLog("The spirit of SM MOD choose to Watch War replays... Let's go !!! :)", $COLOR_INFO)
			WatchWarReplays()
		Case 11
			SetLog("The spirit of SM MOD choose to do nothing... Stupid BOT... :)", $COLOR_INFO)
			DoNothing()
		Case 12
			SetLog("The spirit of SM MOD choose to launch Challenges... Let's go !!! :)", $COLOR_INFO)
			LaunchChallenges()
	EndSwitch
...
Func IsMessagesReplayWindow()
	Local $Result = _Wait4Pixel(760, 112, 0xFFFFFF, 20, 3000, "IsMessagesReplayWindow") ;Wait for Replay Message Window To Be Appear
	Return $Result
EndFunc   ;==>IsMessagesReplayWindow

Func IsDefensesTab()
	Local $Result = _Wait4Pixel(180, 110, 0xF0F4F0, 20, 3000, "IsDefensesTab") ;Wait for Defence To Be Selected
	Return $Result
EndFunc   ;==>IsDefensesTab

Func IsAttacksTab()
	Local $Result = _Wait4Pixel(380, 110, 0xF0F4F0, 20, 3000, "IsAttacksTab") ;Wait for Attack To Be Selected
	Return $Result
EndFunc   ;==>IsAttacksTab

Func IsBestPlayers()
	Local $Result = _Wait4Pixel(530, 60, 0xF0F4F0, 20, 3000, "IsBestPlayers") ;Wait for Best Player Screen To Be Appear
	Return $Result
EndFunc   ;==>IsBestPlayers

Func IsBestClans()
	Local $Result = _Wait4Pixel(350, 60, 0xF0F4F0, 20, 3000, "IsBestClans") ;Wait for Best Clan Screen To Be Appear
	Return $Result
EndFunc   ;==>IsBestClans

Func ChatOpen()
	Local $Result = _Wait4Pixel(330, 382, 0xFFFFFF, 20, 3000, "ChatOpen") ;Wait for Chat To Be Appear
	Return $Result
EndFunc   ;==>ChatOpen
...
Func IsTextBox()
	Local $Result = _Wait4Pixel(190, 710, 0xFFFFFF, 20, 3000, "IsTextBox") ;Wait for Text Box To Be Appear
	Return $Result
EndFunc   ;==>IsTextBox
...
Func IsClanOverview()
	Local $Result = _Wait4Pixel(822, 70, 0xFFFFFF, 20, 3000, "IsClanOverview") ;Wait for Is Clan Overview To Be Appear
	Return $Result
