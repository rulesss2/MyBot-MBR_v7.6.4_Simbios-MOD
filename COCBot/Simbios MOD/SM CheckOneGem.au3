...
Func OneGemBoost()
	...
	If $g_bChkOneGemBoostHeroes Then
		...
		For $index = 0 To 2
			If $g_bFirstStart Or $initBoostTimeHeroes = "" Or $ChkIfBoostHero > 60 Or $g_asHeroHealTime[$index] <= 0 Then
...
Func CheckTroopsOneGem()
	OpenTroopsTab(False, "CheckTroopsOneGem()")
...
Func CheckSpellsOneGem()
	OpenSpellsTab(False, "CheckSpellsOneGem()")
	Local $aBoostBtn = findButton("BoostBarrack")
	If IsArray($aBoostBtn) Then
		ClickP($aBoostBtn)
		_Sleep($DELAYBOOSTBARRACKS1)
