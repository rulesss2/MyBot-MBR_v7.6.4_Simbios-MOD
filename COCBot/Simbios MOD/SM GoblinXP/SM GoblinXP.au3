...
Func SetStatsGoblinsXP()
	...
	Else
		If $g_bDebugSX = False Then Setlog("'Other' account Update Values!", $COLOR_DEBUG)
...
Func SXSetXP($toSet = "")
	...
	If $toSet = "H" Or $toSet = "" Then GUICtrlSetData($g_hLblXPSXWonHour, _NumberFormat($g_iGainedXPHour))
...
Func MainSuperXPHandler()
	If $g_bEnableSuperXP = False Then Return
	If $g_bDebugSetlog Or $g_bDebugSX Then SetLog("Begin MainSuperXPHandler, $g_irbSXTraining=" & $g_irbSXTraining & ", $IsFullArmywithHeroesAndSpells=" & $g_bIsFullArmywithHeroesAndSpells, $COLOR_DEBUG)
	...
	If $g_iGainedXP >= $g_iTxtMaxXPtoGain Then
		...
		If $g_bDebugSX = False Then SetLog("$g_iGainedXP = " & $g_iGainedXP & "|$g_iTxtMaxXPtoGain = " & $g_iTxtMaxXPtoGain, $COLOR_DEBUG)
		$g_bEnableSuperXP = False
		GUICtrlSetState($g_hChkEnableSuperXP, $GUI_UNCHECKED)
	...
	If $g_bDebugSetlog = True Then SetLog("Current Trophy Count: " & $g_aiCurrentLoot, $COLOR_DEBUG) ;Debug
	...
	If $g_bDebugSX = True Then SetLog("$g_iHeroAvailable = " & $g_iHeroAvailable)
	If $g_bDebugSX = True Then SetLog("BK: " & $g_bSXBK & ", AQ: " & $g_bSXAQ & ", GW: " & $g_bSXGW)
	If $g_bDebugSX = True Then SetLog("$canGainXP = " & $g_canGainXP & @CRLF & "1: " & String(IIf($g_bSXBK = $eHeroNone, False, BitAND($g_iHeroAvailable, $eHeroKing) = $eHeroKing)) & ", 2: " & _
	...
	While $g_canGainXP = True
		If WaitForMain() = False Then
			...
			Return False
		EndIf
		If $g_iGainedXP >= $g_iTxtMaxXPtoGain And $g_bFastGoblinXP = False Then
			$g_canGainXP = False
			SetLog("You have Max XP to Gain GoblinXP", $COLOR_DEBUG)
			If $g_bDebugSX = True Then SetLog("$g_iGainedXP = " & $g_iGainedXP & "|$g_iTxtMaxXPtoGain = " & $g_iTxtMaxXPtoGain, $COLOR_DEBUG)
			$g_bEnableSuperXP = False
			GUICtrlSetState($g_hChkEnableSuperXP, $GUI_UNCHECKED)
		...
		If $g_bSkipZoomOutXP = False Then
		...
		If ProfileSwitchAccountEnabled() And $g_canGainXP And $CurrentXPgain >= 50 And $g_bFastGoblinXP = False Then
		...
		If $g_bDebugSX = True Then SetLog("$g_iHeroAvailable = " & $g_iHeroAvailable)
		If $g_bDebugSX = True Then SetLog("BK: " & $g_bSXBK & ", AQ: " & $g_bSXAQ & ", GW: " & $g_bSXGW)
		If $g_bDebugSX = True Then SetLog("While|$g_canGainXP = " & $g_canGainXP & @CRLF & "1: " & String(IIf($g_bSXBK = $eHeroNone, False, BitAND($g_iHeroAvailable, $eHeroKing) = $eHeroKing)) & ", 2: " & _
...
Func CheckForFullArmy()
	If $g_bDebugSX = True Then SetLog("SX|CheckForFullArmy", $COLOR_PURPLE)
	...
	If $g_bDebugSX = True Then Setlog("$g_bIsFullArmywithHeroesAndSpells: " & $g_bIsFullArmywithHeroesAndSpells)
	If $g_bDebugSX = True Then Setlog("1 Pixel : " & _GetPixelColor(391, 126, True))
	If $g_bDebugSX = True Then Setlog("2 Pixel : " & _GetPixelColor(587, 126, True))
	...
	If $g_bIsFullArmywithHeroesAndSpells = False And _
		...
		If $g_bDebugSX = True Then SetLog("SX|CFFA TrainSystem Condi. #1")
		TrainSystem()
	ElseIf $g_bIsFullArmywithHeroesAndSpells = True And $g_bEnableSuperXP = True And $g_irbSXTraining = 1 Then ; Train Troops Before Attack
		If $g_bDebugSX = True Then SetLog("SX|CFFA TrainSystem Condi. #2")
		TrainSystem()
	EndIf

	;ClickP($aAway, 2, 0, "#0346") ;Click Away
	If $g_bDebugSX = True Then SetLog("SX|CheckForFullArmy Finished", $COLOR_PURPLE)
EndFunc   ;==>CheckForFullArmy

Func SafeReturnSX()
	If $g_bDebugSX = True Then SetLog("SX|SafeReturn", $COLOR_PURPLE)
	$g_canGainXP = False
	...
	ElseIf IsInSPPage() Then
		$rExit = ExitSPPage()
	EndIf
	If $g_bDebugSX = True Then SetLog("SX|SafeReturn=" & $rExit)
	Return $rExit
EndFunc   ;==>SafeReturnSX

Func ExitSPPage()
	If $g_bDebugSX = True Then SetLog("SX|ExitSPPage", $COLOR_PURPLE)
	...
	If $Counter >= 200 Then
		...
		Return False
	EndIf
	If $g_bDebugSX = True Then SetLog("SX|ExitSPPage Finished", $COLOR_PURPLE)
	Return True
EndFunc   ;==>ExitSPPage

Func AttackFinishedSX()
	If $g_bDebugSX = True Then SetLog("SX|AttackFinished", $COLOR_PURPLE)
	...
	If $g_bDebugSX = True Then SetLog("SX|AttackFinished Finished", $COLOR_PURPLE)
EndFunc   ;==>AttackFinishedSX

Func GetCurXP($returnVal = "Current")
	If $g_bFastGoblinXP = True Then Return
	;Return getVillageExp(55, 20)
	If $g_bDebugSX = True Then SetLog("SX|GetCurXP", $COLOR_PURPLE)
	Local $ToReturn = "0#0"
	...
	If $g_bDebugSX = True Then SetLog("SX|GetCurXP $XPOCRResult: " & $XPOCRResult, $COLOR_PURPLE)
	...
	If $g_bDebugSX = True Then SetLog("SX|GetCurXP Finished", $COLOR_PURPLE)
...
Func WaitToFinishSuperXP()
	If $g_bDebugSX = True Then SetLog("SX|WaitToFinishSuperXP", $COLOR_PURPLE)
	...
	While 1
		...
		If TimerDiff($BdTimer) >= 120000 Then ; If Battle Started 2 Minutes ago, Then Return
			If $g_bDebugSX = True Then SetLog("SX|WaitToFinishSuperXP TimeOut", $COLOR_RED)
			SafeReturnSX()
			ExitLoop
		EndIf
	WEnd
	If $g_bDebugSX = True Then SetLog("SX|WaitToFinishSuperXP Finished", $COLOR_PURPLE)
	Return True
...
Func ActivateHeroesByDelay($hBdTimer)
	...
	If $tDiff >= $QueenDelay And $QueenDelay <> 0 And $g_ActivatedHeroes[0] = False And $g_iQueenSlot <> -1 And $g_bSXAQ <> $eHeroNone Then
		If $g_bDebugSX = True Then SetLog("SX|Activating Queen Ability After " & Round($tDiff, 3) & "/" & $QueenDelay & " ms(s)")
	...
	If $tDiff >= $WardenDelay And $WardenDelay <> 0 And $g_ActivatedHeroes[1] = False And $g_iWardenSlot <> -1 And $g_bSXGW <> $eHeroNone Then
		If $g_bDebugSX = True Then SetLog("SX|Activating Warden Ability After " & Round($tDiff, 3) & "/" & $WardenDelay & " ms(s)")
	...
	If $tDiff >= $KingDelay And $KingDelay <> 0 And $g_ActivatedHeroes[2] = False And $g_iKingSlot <> -1 And $g_bSXBK <> $eHeroNone Then
		If $g_bDebugSX = True Then SetLog("SX|Activating King Ability After " & Round($tDiff, 3) & "/" & $KingDelay & " ms(s)")
...
Func IsInAttackSuperXP()
	If $g_bDebugSX = True Then SetLog("SX|IsInAttackSuperXP", $COLOR_PURPLE)
	If _ColorCheck(_GetPixelColor(60, 576, True), Hex(0x000000, 6), 20) Then Return True
	If $g_bDebugSX = True Then SetLog("SX|IsInAttackSuperXP=FALSE")
	Return False
EndFunc   ;==>IsInAttackSuperXP

Func IsInSPPage()
	If $g_bDebugSX = True Then SetLog("SX|IsInSPPage", $COLOR_PURPLE)
	Local $rColCheck = _ColorCheck(_GetPixelColor(316, 34, True), Hex(0xFFFFFF, 6), 20)
	If $g_bDebugSX = True Then SetLog("SX|IsInSPPage=" & $rColCheck)
	Return $rColCheck
EndFunc   ;==>IsInSPPage

Func AttackSuperXP()
	If $g_bDebugSX = True Then SetLog("SX|AttackSuperXP", $COLOR_PURPLE)
	If WaitForNoClouds() = False Then
		If $g_bDebugSX = True Then SetLog("SX|ASX|Wait For Clouds = False")
	...
	If $g_bDebugSX = True Then SetLog("SX|AttackSuperXP Finished", $COLOR_PURPLE)
	Return True
...
Func CheckAvailableHeroes()
	...
	If $g_bDebugSX = True Then SetLog("SX|CheckAvailableHeroes=" & $g_canGainXP)
	Return $g_canGainXP
...
Func PrepareSuperXPAttack()
	If $g_bDebugSX = True Then SetLog("SX|PrepareSuperXPAttack", $COLOR_PURPLE)
	...
	If $g_bDebugSX = True Then SetLog("SX|PrepareSuperXPAttack Finished", $COLOR_PURPLE)
	Return $troopsnumber
EndFunc   ;==>PrepareSuperXPAttack

Func CheckEarnedStars($ExitWhileHave = 0) ; If the parameter is 0, will not exit from attack lol
	If $g_bDebugSX = True Then SetLog("SX|CheckEarnedStars", $COLOR_PURPLE)
	Local $starsearned = 0
...
Func ReturnHomeSuperXP()
	...
	While _ColorCheck(_GetPixelColor($EndBattleText[0], $EndBattleText[1], True), Hex($EndBattleText[2], 6), $EndBattleText[3]) = False ; First EndBattle Button
		If $g_bDebugSX = True Then SetLog("SX|RHSX|1-Loop #" & $Counter, $COLOR_DEBUG)
		If _Sleep($DELAYEachCheck) Then Return False
		$Counter += 1
		If $Counter >= $iRetryLimits Then
			If $g_bDebugSX = True Then SetLog("SX|RHSX|First EndBattle Button not found")
			Return False
		EndIf
	WEnd
	...
	While _ColorCheck(_GetPixelColor($EndBattle2Text[0], $EndBattle2Text[1], True), Hex($EndBattle2Text[2], 6), $EndBattle2Text[3]) = False ; Second EndBattle Button
		If $g_bDebugSX = True Then SetLog("SX|RHSX|2-Loop #" & $Counter, $COLOR_DEBUG)
		If _Sleep($DELAYEachCheck) Then Return False
		$Counter += 1
		If $Counter >= $iRetryLimits Then
			If $g_bDebugSX = True Then SetLog("SX|RHSX|Second EndBattle Button not found")
			Return False
		EndIf
	WEnd
	...
	While _ColorCheck(_GetPixelColor($ReturnHomeText[0], $ReturnHomeText[1], True), Hex($ReturnHomeText[2], 6), $ReturnHomeText[3]) = False ; Last - Return Home Button
		If $g_bDebugSX = True Then SetLog("SX|RHSX|3-Loop #" & $Counter, $COLOR_DEBUG)
		If _Sleep($DELAYEachCheck) Then Return False
		$Counter += 1
		If $Counter >= $iRetryLimits Then
			If $g_bDebugSX = True Then SetLog("SX|RHSX|Last Return Home Button not found")
			Return False
		EndIf
	WEnd
	...
	$Counter = 0 ; Reset Counter
	While 1
		If $g_bDebugSX = True Then SetLog("SX|RHSX|4-Loop #" & $Counter, $COLOR_DEBUG)
...
Func WaitForNoClouds()
	If $g_bDebugSX = True Then SetLog("SX|WaitForNoClouds", $COLOR_PURPLE)
	Local $i = 0
	ForceCaptureRegion()
	While _ColorCheck(_GetPixelColor(60, 576, True), Hex(0x000000, 6), 20) = False
		...
		If $i >= 120 Or isProblemAffect(True) Then ; Wait 30 seconds then restart bot and CoC
			...
			Return False
		EndIf
		If $g_bDebugSX = True Then SetLog("SX|WFNC|Loop #" & $i)
		ForceCaptureRegion() ; ensure screenshots are not cached
	WEnd
	If $g_bDebugSX = True Then SetLog("SX|WaitFornoClouds Finished", $COLOR_PURPLE)
	Return True
EndFunc   ;==>WaitForNoClouds

Func OpenGoblinPicnic()
	If $g_bDebugSX = True Then SetLog("SX|OpenGoblinPicnic", $COLOR_PURPLE)
	...
	If $g_bDebugSX = True Then SetLog("SX|OGP|Clicking On GP Text: " & $rDragToGoblinPicnic[0] & ", " & $rDragToGoblinPicnic[1])
	...
	If $g_bDebugSX = True Then SetLog("SX|OGP|Clicking On Attack Btn: " & $rDragToGoblinPicnic[0] & ", " & $rDragToGoblinPicnic[1] + 78)
...
Func IsInGoblinPicnic($Retry = True, $maxRetry = 30, $timeBetweenEachRet = 300)
	If $g_bDebugSX = True Then SetLog("SX|IsInGoblinPicnic", $COLOR_PURPLE)
	Local $Found = False
	Local $Counter = 0
	...
	While $Found = False
		...
		If $g_bDebugSX = True Then SetLog("SX|IGP|$result=" & $result)
		...
		EndIf
	WEnd
	If $g_bDebugSX = True Then SetLog("SX|IsGoblinPicnic=" & $Found, $COLOR_PURPLE)
	Return $Found
EndFunc   ;==>IsInGoblinPicnic

Func IsGoblinPicnicLocked($FoundCoord)
	If $g_bDebugSX = True Then SetLog("SX|IsGoblinPicnicLocked", $COLOR_PURPLE)
	...
	Switch $posInSinglePlayer
		Case "END"
			While Not (IsArray($rIsGoblinPicnicFound))
				If $g_bDebugSX = True Then SetLog("SX|DTGP|Drag from End Loop #" & $Counter)
		...
		Case "FIRST"
			While Not (IsArray($rIsGoblinPicnicFound))
				If $g_bDebugSX = True Then SetLog("SX|DTGP|Drag from First Loop #" & $Counter)
...
Func IsGoblinPicnicFound()
	If $g_bDebugSX = True Then SetLog("SX|IsGoblinPicnicFound", $COLOR_PURPLE)
	...
	If $g_bDebugSX = True Then SetLog("SX|IGPF|$result=" & $result)
	If StringLen($result) < 3 And StringInStr($result, "|") = 0 Then
		If $g_bDebugSX = True Then SetLog("SX|IGPF|Return False")
		Return False
	EndIf
	...
	$ToReturn[0] += 554
	$ToReturn[1] += 120
	If $g_bDebugSX = True Then SetLog("SX|IGPF Return $ToReturn[2] = [0]=" & $ToReturn[0] & ",[1]=" & $ToReturn[1])
	Return $ToReturn
EndFunc   ;==>IsGoblinPicnicFound

Func DragToEndSinglePlayer()
	If $g_bDebugSX = True Then SetLog("SX|DragToEndSinglePlayer", $COLOR_PURPLE)
	Local $rColCheckEnd = _ColorCheck(_GetPixelColor(670, 695, True), Hex(0x393224, 6), 20)
	Local $Counter = 0
	While $rColCheckEnd = False
		If $g_bDebugSX = True Then SetLog("SX|DTESP|Loop #" & $Counter)
	...
	WEnd
	If $Counter = 15 Then
		If $g_bDebugSX = True Then SetLog("SX|DTESP|Return False")
		Return False
	EndIf
	If $g_bDebugSX = True Then SetLog("SX|DTESP|Return True")
	Return True
EndFunc   ;==>DragToEndSinglePlayer

Func GetPositionInSinglePlayer()
	If $g_bDebugSX = True Then SetLog("SX|GetPositionInSinglePlayer", $COLOR_PURPLE)
	...
	While _ColorCheck(_GetPixelColor(621, 665, True), Hex(0xFFFFFF, 6), 10) And _ColorCheck(_GetPixelColor(663, 662, True), Hex(0xFFFFFF, 6), 10)
		...
		$Counter += 1
		If $Counter > 100 Then
			If $g_bDebugSX = True Then SetLog("SX|GPISP|Available Loot Not Hidden, Returning")
			ExitLoop
		EndIf
	WEnd
	...
	If $rColCheckEnd Then
		If $g_bDebugSX = True Then SetLog("SX|GPISP|Return END")
		Return "END"
	Else
		Local $rColCheckFirst = _ColorCheck(_GetPixelColor(585, 4, True), Hex(0x372D22, 6), 20)
		If $rColCheckFirst Then
			If $g_bDebugSX = True Then SetLog("SX|GPISP|Return FIRST")
			Return "FIRST"
		Else
			If $g_bDebugSX = True Then SetLog("SX|GPISP|Return MIDDLE")
			Return "MIDDLE"
		EndIf
	EndIf
...
Func OpenSinglePlayerPage()
	If $g_bDebugSX = True Then SetLog("SX|OpenSinglePlayerPage", $COLOR_PURPLE)

	If WaitForMain(True, 50, 300) = False Then
		If $g_bDebugSX = True Then SetLog("SX|MainPage Not Displayed to Open SingleP")
		Return False
	EndIf
