		...
		If $i > 0 Then
			...
			If BB_PrepareAttack() Then
				...
				For $i = 0 To 5
					...
					EndIf
					$j = 0
					If ($i >= 0) Then ; EDITED By SM MOD
						$cPixColor = _GetPixelColor($aTroopSlot[0], $aTroopSlot[1], True)
						If _Sleep($DELAYRUNBOT1) Then Return
						If BB_ColorCheck($aTroopSlot, $aSlotActive) Then
							If $bDegug And $i > 0 Then SetLog("BB: Click Next Slot, color: " & $cPixColor & " [ " & String($i + 1) & " ]", $COLOR_DEBUG) ; EDITED By SM MOD
							If $bDegug And $i = 0 Then SetLog("BB: Click 1st Slot, color: " & $cPixColor & " [ " & String($i + 1) & " ]", $COLOR_DEBUG) ; ADDED By SM MOD
							ClickP($aTroopSlot, 1, 0, "#0000")
						Else
							If $i = 0 Then SetLog("BB: Can't Find 1st Slot, color: " & $cPixColor & " [ " & String($i + 1) & " ]", $COLOR_DEBUG) ; EDITED By SM MOD
							If $i > 0 Then SetLog("BB: Can't Find Next Slot, color: " & $cPixColor & " [ " & String($i + 1) & " ]", $COLOR_DEBUG) ; ADDED By SM MOD
					...
					If $bContinue Then
						...
						; If $iTroopsTo < 4 Then $iTroopsTo = 4 ; REMOVED By SM MOD
