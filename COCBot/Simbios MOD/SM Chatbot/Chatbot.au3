...
Func ChatbotChatOpen() ; open the chat area
	...
	If _CheckColorPixel($g_aButtonChatWindowOpen[2], $g_aButtonChatWindowOpen[3], $g_aButtonChatWindowOpen[4], $g_aButtonChatWindowOpen[5], $g_bCapturePixel, "ChatbotChatOpen") Then ;Check if Chat Button Exists
		...
		If _Wait4Pixel($g_aButtonChatWindowClose[2], $g_aButtonChatWindowClose[3], $g_aButtonChatWindowClose[4], $g_aButtonChatWindowClose[5], 1500) = False Then ;Wait for chat to be open if it is not opened in 1500 sec show error
...
Func ChatbotSelectClanChat() ; select clan tab
	...
	If _Wait4Pixel($g_aButtonChatSelectTabClan[2], $g_aButtonChatSelectTabClan[3], $g_aButtonChatSelectTabClan[4], $g_aButtonChatSelectTabClan[5], 1500) = False Then ;Wait for clan chat tab to be selected if it is not opened in 1500 sec show error
...
Func ChatbotCheckIfUserIsInClan() ; check if user is in a clan before doing chat
	If _Wait4Pixel($g_aButtonChatJoinClan[0], $g_aButtonChatJoinClan[1], $g_aButtonChatJoinClan[2], $g_aButtonChatJoinClan[3], 500) = True Then ;Wait for 500ms to check if user is in clan or not
...
Func ChatbotSelectGlobalChat() ; select global tab
	...
	If _Wait4Pixel($g_aButtonChatSelectTabGlobal[2], $g_aButtonChatSelectTabGlobal[3], $g_aButtonChatSelectTabGlobal[4], $g_aButtonChatSelectTabGlobal[5], 1500) = False Then ;Wait for global tab to be selected if it is not opened in 1500 sec show error
...
Func ChatbotChatClose() ; close chat area
	...
	If _Wait4PixelGone($g_aButtonChatWindowClose[2], $g_aButtonChatWindowClose[3], $g_aButtonChatWindowClose[4], $g_aButtonChatWindowClose[5], 1500) Then ;Wait for chat dialog to be disappear if it is not disappeared in 1500 sec show error
...
Func ChatbotSelectChatInput($fromTab) ; select the textbox for global chat or Clan Chat
	...
	If _Wait4Pixel($g_aButtonChatSelectTextBox[2], $g_aButtonChatSelectTextBox[3], $g_aButtonChatSelectTextBox[4], $g_aButtonChatSelectTextBox[5], 1000) = False Then ;Wait for Textbox to be appear if it is not opened in 1500 sec show error
...
Func ChatbotSendChat($fromTab) ; click send for global or clan chat
	...
	If _Wait4PixelGone($g_aButtonChatSendButton[2], $g_aButtonChatSendButton[3], $g_aButtonChatSendButton[4], $g_aButtonChatSendButton[5], 1000) Then ;Wait for Textbox to be disappear if it is not gone in 1500 sec show error
		Return True
	Else
...
Func ChatbotSettingOpen() ; Open the Settings
	...
	If _CheckColorPixel($g_aButtonLangSetting[2], $g_aButtonLangSetting[3], $g_aButtonLangSetting[4], $g_aButtonLangSetting[5], $g_bCapturePixel, "ChatbotSettingOpen") Then ;Check if Setting Button Exists
		...
		If _Wait4Pixel($g_aButtonLangSettingClose[2], $g_aButtonLangSettingClose[3], $g_aButtonLangSettingClose[4], $g_aButtonLangSettingClose[5], 1500) = False Then ;Wait for Settings button to open if it is not opened in 1500 sec show error
...
Func ChatbotClickLanguageButton() ; Click on language button in settings
	...
	If _Wait4Pixel($g_aButtonLangSettingBackLang[2], $g_aButtonLangSettingBackLang[3], $g_aButtonLangSettingBackLang[4], $g_aButtonLangSettingBackLang[5], 1500) = False Then ;Wait for language page to be opened it is not opened in 1500 sec show error
...
Func ChatbotSettingClose() ; close settings
	...
	If _Wait4PixelGone($g_aButtonLangSettingClose[2], $g_aButtonLangSettingClose[3], $g_aButtonLangSettingClose[4], $g_aButtonLangSettingClose[5], 1500) Then ;Wait for setting dialog to be disappear if it is not disappeared in 1500 sec show error
...
Func ChangeLanguageForChatBot($lButtonLanguage, $lngMsg) ; Select Language Which You Want To Switch and click on that
	...
	If _Wait4Pixel($g_aButtonLangSettingOk[2], $g_aButtonLangSettingOk[3], $g_aButtonLangSettingOk[4], $g_aButtonLangSettingOk[5], 1500) = False Then ;Wait for language Dialog to be appear in 1500 sec show error
...
Func ChangeLanguagePressOk($lngMsg) ;Press OK On Lanugage Select Dialog
	...
	If _Wait4PixelGone($g_aButtonLangSettingOk[2], $g_aButtonLangSettingOk[3], $g_aButtonLangSettingOk[4], $g_aButtonLangSettingOk[5], 1500) Then ;Wait for language Dialog to be appear in 1500 sec show error
