...
Func ChatbotChatOpen() ; open the chat area
	...
	If _CheckColorPixel($g_aButtonChatWindowOpen[2], $g_aButtonChatWindowOpen[3], $g_aButtonChatWindowOpen[4], $g_aButtonChatWindowOpen[5], $g_bCapturePixel, "ChatbotChatOpenChk") Then ;Check if Chat Button Exists
		...
		If _Wait4Pixel($g_aButtonChatWindowClose[2], $g_aButtonChatWindowClose[3], $g_aButtonChatWindowClose[4], $g_aButtonChatWindowClose[5], 5000, "ChatbotChatOpen") = False Then ;Wait for chat to be open if it is not opened in 5 sec show error
...
Func ChatbotCheckIfUserIsInClan() ; check if user is in a clan before doing chat
	If _Wait4Pixel($g_aButtonChatJoinClan[0], $g_aButtonChatJoinClan[1], $g_aButtonChatJoinClan[2], $g_aButtonChatJoinClan[3], 1000, "ChatbotCheckIfUserIsInClan") = True Then ;Wait for 500ms to check if user is in clan or not
...
Func ChatbotChatClose() ; close chat area
	...
	If _Sleep(300) Then Return ; Delay Added Just For Human Like Behavior otherwise not needed
	If _Wait4PixelGone($g_aButtonChatWindowClose[2], $g_aButtonChatWindowClose[3], $g_aButtonChatWindowClose[4], $g_aButtonChatWindowClose[5], 5000, "ChatbotChatClose") Then ;Wait for chat dialog to be disappear if it is not disappeared in 5 sec show error
...
Func ChatbotSelectChatInput($fromTab) ; select the textbox for global chat or Clan Chat
	...
	If _Sleep(300) Then Return ; Delay Added Just For Human Like Behavior otherwise not needed
	If _Wait4Pixel($g_aButtonChatSelectTextBox[2], $g_aButtonChatSelectTextBox[3], $g_aButtonChatSelectTextBox[4], $g_aButtonChatSelectTextBox[5], 3000, "ChatbotSelectChatInput") = False Then ;Wait for Textbox to be appear if it is not opened in 5 sec show error
		; When Emulator Navigation Buttons Set To Bottom chatbox position is different
		If _Wait4Pixel($g_aButtonChatSelectTextBoxBottomNav[2], $g_aButtonChatSelectTextBoxBottomNav[3], $g_aButtonChatSelectTextBoxBottomNav[4], $g_aButtonChatSelectTextBoxBottomNav[5], 3000, "ChatbotSelectChatInput") = False Then ;Wait for Textbox to be appear if it is not opened in 5 sec show error
...
Func ChatbotSettingOpen() ; Open the Settings
	...
	If _CheckColorPixel($g_aButtonLangSetting[2], $g_aButtonLangSetting[3], $g_aButtonLangSetting[4], $g_aButtonLangSetting[5], $g_bCapturePixel, "ChatbotSettingOpenChk") Then ;Check if Setting Button Exists
		...
		If _Sleep(300) Then Return ; Delay Added Just For Human Like Behavior otherwise not needed
		If _Wait4Pixel($g_aButtonLangSettingClose[2], $g_aButtonLangSettingClose[3], $g_aButtonLangSettingClose[4], $g_aButtonLangSettingClose[5], 5000, "ChatbotSettingOpen") = False Then ;Wait for Settings button to open if it is not opened in 5 sec show error
...
Func ChatbotClickLanguageButton() ; Click on language button in settings
	...
	If _Sleep(300) Then Return ; Delay Added Just For Human Like Behavior otherwise not needed
	If _Wait4Pixel($g_aButtonLangSettingBackLang[2], $g_aButtonLangSettingBackLang[3], $g_aButtonLangSettingBackLang[4], $g_aButtonLangSettingBackLang[5], 5000, "ChatbotClickLanguageButton") = False Then ;Wait for language page to be opened it is not opened in 5 sec show error
...
Func ChatbotSettingClose() ; close settings
	...
	If _Sleep(300) Then Return ; Delay Added Just For Human Like Behavior otherwise not needed
	If _Wait4PixelGone($g_aButtonLangSettingClose[2], $g_aButtonLangSettingClose[3], $g_aButtonLangSettingClose[4], $g_aButtonLangSettingClose[5], 5000, "ChatbotSettingClose") Then ;Wait for setting dialog to be disappear if it is not disappeared in 5 sec show error
...
Func ChangeLanguageForChatBot($lButtonLanguage, $lngMsg) ; Select Language Which You Want To Switch and click on that
	...
	If _Sleep(300) Then Return ; Delay Added Just For Human Like Behavior otherwise not needed
	If _Wait4Pixel($g_aButtonLangSettingOk[2], $g_aButtonLangSettingOk[3], $g_aButtonLangSettingOk[4], $g_aButtonLangSettingOk[5], 5000, "ChangeLanguageForChatBot") = False Then ;Wait for language Dialog to be appear in 5 sec show error
...
Func ChangeLanguagePressOk($lngMsg) ;Press OK On Lanugage Select Dialog
	...
	If _Sleep(300) Then Return ; Delay Added Just For Human Like Behavior otherwise not needed
	If _Wait4PixelGone($g_aButtonLangSettingOk[2], $g_aButtonLangSettingOk[3], $g_aButtonLangSettingOk[4], $g_aButtonLangSettingOk[5], 5000, "ChangeLanguagePressOk") Then ;Wait for language Dialog to be appear in 5 sec show error
