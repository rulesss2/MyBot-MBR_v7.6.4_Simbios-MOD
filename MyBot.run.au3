...
;==================Themes ADDED by SM MOD ====
#include "Themes\_UskinLibrary.au3"
_Uskin_LoadDLL()
;===========================

Func UpdateBotTitle()
	Local $sTitle = "My Bot " & $g_sBotVersion & " Simbios MOD " & $g_sModversion
...
Func FinalInitialization(Const $sAI)
	...
	SetLog(GetTranslatedFileIni("MBR GUI Design - Loading", "Msg_Theme_instance_30", "Theme used", $sAI), $COLOR_SUCCESS)
	...
	SetLog("_________" & " [  MyBot Simbios MOD  ]" & "_________", $COLOR_MONEYGREEN, "Impact", 14)
	...
	While 1
		...
		If $g_bIsClientSyncError = False And $g_bIsSearchLimit = False And ($g_bQuickAttack = False) Then
			...
			If IsSearchAttackEnabled() Then ; if attack is disabled skip reporting, requesting, donating, training, and boosting
				Local $aRndFuncList = ['ReplayShare', 'NotifyReport', 'DonateCC,Train', 'RequestCC', 'CollectFreeMagicItems', 'HeroTimeBoost']
				While 1
					If $g_bRunState = False Then Return
					If $g_bRestart = True Then ContinueLoop 2 ; must be level 2 due to loop-in-loop
					If UBound($aRndFuncList) > 1 Then
			...
			EndIf
			MainSuperXPHandler() ; ADDED By SM MOD
...
Func _Idle() ;Sequence that runs until Full Army
	...
	ForecastSwitch() ; ADDED By SM MOD
	...
	While $g_bIsFullArmywithHeroesAndSpells = False
		...
		BotHumanization() ; Bot Humanization - ADDED BY SM MOD
		...
		If $g_iCommandStop = -1 Then
			If $g_iActualTrainSkip < $g_iMaxTrainSkip Then
				MainSuperXPHandler() ; ADDED By SM MOD
				If CheckNeedOpenTrain($g_sTimeBeforeTrain) Then
					TrainSystem()
		...
		If $g_bCanRequestCC = True And BalanceRecRec(True) Then RequestCC() ; EDITED By SM MOD
...
Func _RunFunction($action)
	...
	Switch $action
		...
		Case "DonateCC"
			If $g_iActiveDonate And $g_bChkDonate Then
				If SkipDonateNearFullTroops(True) = False Then DonateCC() ;  was And BalanceDonRec(True) EDITED By SM MOD
				If _Sleep($DELAYRUNBOT1) = False Then checkMainScreen(False)
			EndIf
			;------------------ADDED By SM MOD - START------------------
		Case "SendChat"
		...
		Case "BoostWarden"
			BoostWarden()
			;------------------CUSTOM LOGIC By SM MOD - START------------------
		Case "BoostAll" ; EDITED By SM MOD
			BoostAllWithMagicSpell() ; EDITED By SM MOD
		Case "LabCheck"
			If $g_iChkLabCheck = 0 Then ; ADDED By SM MOD
				LabGuiDisplay()
				_Sleep($DELAYRUNBOT3)
			EndIf ; ADDED By SM MOD
		Case "RequestCC"
...
Func FirstCheck()
	...
	BotHumanization() ; Added By SM MOD

	If $g_bEnableSuperXP = True And $g_irbSXTraining = 2 Then ;Added By SM MOD when Super Xp Only Farm Option is on skip all and just do the Goblin Xp Farming
		MainSuperXPHandler()
		Return
	EndIf
	;------------------CUSTOM LOGIC By SM MOD - START------------------
	...
	MainKickout()
	VillageReport()
	If $g_bReqCCFirst = True And BalanceRecRec(True) Then RequestCC()
